...
function CreateDelivery() {
  ...
  const generateDeliveryNumber = async () => {
    try {
      const response = await fetch('http://your-backend-url/api/deliveries', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
      if (response.ok) {
        ...
        setFormData(prev => ({
          ...prev,
          deliveryNumber: `DEL-${String(nextNumber).padStart(3, '0')}`
        }));
      }
    } catch (error) {
      setFormData(prev => ({
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <h1 className="text-2xl font-bold">Create New Delivery</h1>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-gray-700 mb-1">Delivery Number *</label>
                  <input
                    type="text"
                    name="deliveryNumber"
